...
//      std::vector<MPI_Request > put_req((Src.DataVol ()/bsize));
        size_t nreq = (Src.DataVol () / bsize);
        MPI_Request put_req[nreq];
...
#else
//          MPI_Request req_tmp;
            MPI_Rput (send_buf[k] + mem_size * j,
                      bsize * mem_size * sizeof (DATA), MPI_BYTE, target,
                      offset, bsize * mem_size * sizeof (DATA), MPI_BYTE,
                      recv_win, &put_req[j / bsize]);
//                 bsize*mem_size * sizeof (DATA), MPI_BYTE, recv_win,(put_req.data()+(j/bsize)));
