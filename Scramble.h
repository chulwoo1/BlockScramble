...
//      bsize=1;
    ...
    for (int recv_i = 0; recv_i < recv_max; recv_i++) {
      MPI_Win recv_win;
      MPI_Win_create (recv_buf[recv_i],
		      sizeof (DATA) * mem_size * Dest.DataVol (),
	...
	if (!rank)
	  if (verb > 5)
	    std::cout << "SrcIndex " << SrcIndex[k] << " Src.BlockTotal " <<
	      Src.BlockTotal () << " Src.BlockIndex " << Src.BlockIndex () <<
	      std::endl;
...
#pragma omp parallel for
	for (size_t j = 0; j < Src.DataVol (); j += bsize) {
	  if (verb > 5)
	    std::cout << j << " : thread " << omp_get_thread_num () << " of " <<
...
#pragma omp critical
	  {
	    ...
	    if (verb > 6)
	      std::cout << *this << "MPI_Put: send_buf[" << k << "]+" <<
		mem_size * j << " " << bsize * mem_size *
		sizeof (DATA) << " : " << target << " " << offset << " " <<
		bsize * mem_size * sizeof (DATA) << std::endl;
	  }
#if 0
...
//More optimized version for  contiguous receive buffer
  void run (BlockGeometry & Src,
	    std::vector < int >&SrcIndex,
	    std::vector < DATA * >send_buf, BlockGeometry & Dest,
	    DATA * recv_buf, int recv_max)
  {
...
//    int recv_max = recv_buf.size ();
    ...
    MPI_Win_create (recv_buf,
		    (MPI_Aint) recv_max * sizeof (DATA) * mem_size *
		    Dest.DataVol (), (MPI_Aint) sizeof (DATA) * mem_size,
		    MPI_INFO_NULL, *mpi_comm, &recv_win);
    MPI_Win_fence (MPI_MODE_NOPRECEDE, recv_win);
    for (int recv_i = 0; recv_i < recv_max; recv_i++) {
...
//#pragma omp parallel for
	for (size_t j = 0; j < Src.DataVol (); j += bsize) {
	  ...
	  size_t target = CoorToIndex (DestNodeCoor, GlobalDim);
	  size_t offset = CoorToIndex (DestSiteCoor, Dest.DataDim);
	  offset += (size_t) recv_i+ Dest.DataVol();
	  if (verb > 3 && !rank)
	    if (target == 0 && offset == 0) {
	      printf
		("target 0 offset 0 Dest.DataVol %d DestWrap %d Index %d DestNodeCoor %d %d %d %d DesSiteCoor %d %d %d %d\n", Dest.DataVol(),
		 DestWrap, SrcIndex[k], DestNodeCoor[0], DestNodeCoor[1],
		 DestNodeCoor[2], DestNodeCoor[3], DestSiteCoor[0],
		 DestSiteCoor[1], DestSiteCoor[2], DestSiteCoor[3]);
	    }
